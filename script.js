...
const veryOldInventors = inventors.filter((inventor) => {
    return inventor = inventors.year > 1500 && inventors.year < 1599;
...
const sortedByBirthYear = inventors.some((inventor) => {
...
const firstName = inventors.map((inventor) => {
  return inventor.first.split();
})

const lastName = inventors.map((inventor) => {
  return inventor.first.split();
})
